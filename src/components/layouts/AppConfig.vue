      ...
      <div class="p-3">
          ...
          <div class="flex justify-content-between">
            <div class="field-radiobutton flex-1">
              <RadioButton
                name="topbarTheme"
                value="light"
                v-model="layoutConfig.topbarTheme"
                :disabled="colorScheme !== 'light'"
                id="menuTheme1"
        ...
        <div class="flex flex-wrap row-gap-3">
          <div v-for="(theme, i) in componentThemes" :key="i" class="w-3">
            ...
            >
              <i
                v-if="theme.name === layoutConfig.theme"
...
const { setScale, layoutConfig, layoutState, onConfigSidebarToggle } =
  useLayout();
...
watch(layoutConfig.value.menuMode, (newVal) => {
  if (newVal === "static") {
    layoutState.value.staticMenuDesktopInactive = false;
  }
});

const colorScheme = ref(layoutConfig.value.colorScheme);
...
const changeColorScheme = (colorScheme) => {
  ...
  replaceLink(themeLink, newHref, () => {
    ...
    layoutConfig.value.topbarTheme = colorScheme;
  });
};
...
const changeTheme = (theme) => {
  ...
  const newHref = themeHref?.replace(layoutConfig.value.theme, theme);
